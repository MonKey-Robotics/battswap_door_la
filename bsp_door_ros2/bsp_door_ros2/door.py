...
# from bsp_msgs.action import DoorControl
# import can
# import time
...
#         feedback_msg = DoorControl.Feedback()
...
import rclpy
from rclpy.node import Node
from bsp_msgs.srv import DoorControl  # Update with the correct service definition
import can
import time

class MotorService(Node):
    def __init__(self):
        super().__init__('bsp_door_service')
        ...
        self.srv = self.create_service(DoorControl, 'bsp_door_control', self.control_callback)
        self.get_logger().info("BSP door control service is ready.")
    ...
    def control_callback(self, request, response):
        ...
        if command == "open":
            self.get_logger().info("Extending actuator... (Opening Door)")
            ...
            response.message = "Actuator extended. Door Opened" if success else "Failed to extend actuator."

        elif command == "close":
            self.get_logger().info("Retracting actuator... (Closing Door)")
            ...
            response.success = success
            response.message = "Actuator retracted. Door Closed" if success else "Failed to retract actuator."
